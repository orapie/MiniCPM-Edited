...
val llamaSourceDir = localProperties.getProperty("llama.src.dir")
...
tasks.register("buildGgmlCpu_v86") {
    ...
    doLast {
        ...
        val ninja = "$sdkRoot/cmake/4.1.2/bin/ninja"
        val toolchain = "$sdkRoot/ndk/27.0.12077973/build/cmake/android.toolchain.cmake"
        val bd = File(project.layout.buildDirectory.asFile.get(), "v86-cmake-ninja/arm64-v8a")
        bd.mkdirs()
        ...
        runCmd(
            ...
            *listOfNotNull(llamaSourceDir?.let { "-DLLAMA_SRC=${file(it).absolutePath.replace('\\', '/')}" }).toTypedArray(),
...
afterEvaluate {
    // Debug runs can safely use the baseline library; LlamaEngine already
    // falls back when the optional v86 variant is absent.
    tasks.findByName("mergeReleaseJniLibFolders")?.dependsOn("buildGgmlCpu_v86")
}
